...
install_cni() {
  ...
  kubectl apply -f "${SCRIPT_DIR}/flannel/kube-flannel.yaml"
...
install_local_path_provisioner() {
  ...
  kubectl apply -f "${SCRIPT_DIR}/local-path/local-path-storage.yaml"
...
install_xyne() {
  ...
  log "Pre-pulling xyne/postgis images via Docker then importing into containerd..."
  for image in \
    "postgis/postgis:15-3.5-alpine" \
  ...
  log "Applying Istio destination rules before app starts..."
