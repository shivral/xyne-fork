...
VM_NAME="${VM_NAME:-xyne-k8s-rhel-test}"
...
create_vm() {
  log "Creating VM '$VM_NAME' ($MACHINE_TYPE) in $ZONE with RHEL 9..."
...
run_install() {
  log "Installing tmux on VM..."
  ssh "sudo dnf install -y -q tmux"
  
  log "Starting install-rhel.sh in tmux on VM (logs → /tmp/install.log)..."
  ssh "chmod +x /tmp/k8s/install-rhel.sh && tmux new-session -d -s install 'sudo bash /tmp/k8s/install-rhel.sh 2>&1 | tee /tmp/install.log; echo \"EXIT_CODE=\$?\" >> /tmp/install.log'"
